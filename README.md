...
The mental frame before the tools: how to think about a test, so that the automation built later has something to stand on. The distinction between testing and checking — Michael Bolton and James Bach's, from Rapid Software Testing — the place of risk and oracles, and the techniques that turn guessing into choosing — equivalence partitioning and boundary values — knowing what is worth checking before automating anything.
